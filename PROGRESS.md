...
F07 added a draw-only recycled population of 28 subtle neon decorations plus a capped set of three procedurally placed Health, Magnet, and Bomb pickups. Health uses a clamped signal-emitting player heal, Magnet snapshots active XP coins and makes them fly to the player before their normal collection path awards XP, and Bomb snapshots only `normal_enemies` and uses their normal damage/death/XP-drop path. On 2026-09-01, import/parser, F00–F07 verification, a 600-frame gameplay smoke run, and `git diff --check` passed. Rendered 1152×648 origin and far-traveled frames were inspected successfully; focused verification also proves annulus placement, node recycling, all caps, type readability, Magnet travel and later-coin exclusion, merged XP carry-over and upgrade pause, boss-safe group filtering, defeat pause, replenishment, and clean restart.
...
Post-roadmap user difficulty feedback on 2026-09-01 reduced projectile speed from `720` to `480` and restored XP attraction from the temporary viewport-wide `720` tuning to the short-range `150` default. Distant coins now remain in place until approached. Collecting Magnet snapshots current coins, marks them for visible flight at normal coin speed, and awards their XP only as they reach the player; later coins are unaffected. The F03/F04/F07 focused checks protect this boundary.
...
F00–F09 are complete and the planned Pulsefall prototype roadmap is finished. Post-roadmap difficulty feedback is implemented with `480` projectile speed and short-range `150` XP attraction; Magnet makes only the currently active coins visibly fly to the player and collect on arrival. The final tree passes Godot 4.7.1 import/parser, `verify_f00.gd` through `verify_f09.gd`, a 3,600-frame main-scene smoke run, and `git diff --check`. The recorded timing run predates this harder adjustment, so the next useful action is a fresh hand-operated balance playtest.
